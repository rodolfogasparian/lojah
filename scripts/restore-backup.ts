...
const SOURCE_BUCKET = "products-backup";
const DEST_BUCKET = "products";
...
async function main() {
  ...
  console.log(`📋 ${total} arquivo(s) encontrado(s). Iniciando restauração...\n`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    console.log(`Restaurando ${i + 1}/${total}: ${nome}...`);
    ...
    if (uploadError) {
      console.warn(`  ⚠️  Falha ao restaurar "${nome}": ${uploadError.message}`);
      continue;
    }
  ...
  console.log(`\n✅ ${restaurados} arquivo(s) restaurados para ${DEST_BUCKET}`);
}
