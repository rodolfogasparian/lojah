...
export async function generateMetadata({
  ...
  const seller = await db.sellerProfile.findFirst({
    where: { slug, company: { slug: "atlantica" } },
    select: { name: true },
  });
  const name = seller?.name ?? "Atlântica Natural";
  return {
    title: `Catálogo Digital Atlântica Natural | Indicação de ${name}`,
    description:
      "Tenha seu catálogo Atlântica Natural no celular, sempre atualizado. Link personalizado, cartão virtual e página de revenda — tudo em um só lugar.",
  };
}

export default async function AtlanticaSellerPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const waIndividual = phone
    ? `https://wa.me/${waPhone(phone)}?text=${encodeURIComponent("Olá! Vi sua indicação e quero contratar o catálogo individual por R$67/ano.")}`
    : null;
  const waPack = phone
    ? `https://wa.me/${waPhone(phone)}?text=${encodeURIComponent("Olá! Vi sua indicação e quero contratar o pack de 10 catálogos por R$370.")}`
    : null;
  const waFinal = phone
    ? `https://wa.me/${waPhone(phone)}?text=${encodeURIComponent("Olá! Tenho interesse em ter meu catálogo digital da Atlântica Natural. Pode me ajudar?")}`
    : null;
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <div className="flex gap-3 flex-wrap">
                {waIndividual ? (
                  <a href={waIndividual} target="_blank" rel="noopener noreferrer">
                    <button
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            >
              ...
              {waPack ? (
                <a href={waPack} target="_blank" rel="noopener noreferrer">
          ...
          {phone ? (
            <p className="text-center text-xs text-neutral-400 mt-6">
              Fale com <span className="text-[#00ff88] font-medium">{seller.name}</span> · {formatPhone(phone)}
            </p>
          ) : null}
        </div>
      </section>
