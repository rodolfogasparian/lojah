      ...
      <div className="p-3 flex flex-col flex-1">
        ...
        <div className="mt-auto">
          ...
          <div className="flex gap-1.5 mt-2.5">
            <a href={whatsappUrl} target="_blank" rel="noreferrer" className="flex-1 h-9 rounded-lg border border-border bg-card text-foreground text-[11px] font-semibold flex items-center justify-center gap-1.5">
              <MessageCircle className="size-3.5 text-green-500" /> Eu quero
            </a>
            <button type="button" aria-label="Compartilhar" onClick={() => onShare?.(product.id)} className="size-9 rounded-lg border border-border bg-card grid place-items-center text-muted-foreground">
          ...
          {qty > 0 ? (
            <div className="mt-2 h-10 rounded-lg bg-primary text-primary-foreground flex items-center justify-between px-2">
              <button type="button" onClick={() => onRemove(product.id)} className="size-7 rounded-md bg-white/10 grid place-items-center"><Minus className="size-3.5" /></button>
              <span className="text-xs font-bold">{qty} no carrinho</span>
              <button type="button" onClick={() => onAdd(product.id)} className="size-7 rounded-md bg-white/10 grid place-items-center"><Plus className="size-3.5" /></button>
            </div>
          ) : (
            <button type="button" onClick={() => onAdd(product.id)} className="w-full mt-2 h-10 rounded-lg bg-primary text-primary-foreground text-[11px] font-bold flex items-center justify-center gap-1.5">
